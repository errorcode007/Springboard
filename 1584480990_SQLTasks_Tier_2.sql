...
SELECT DISTINCT CONCAT( firstname, ' ', surname ) AS mem_name, f.name AS facility
FROM Members AS m
LEFT JOIN Bookings AS b ON m.memid = b.memid
INNER JOIN Facilities AS f ON b.facid = f.facid
WHERE b.facid
IN ( SELECT facid FROM Facilities WHERE name LIKE ('Tennis%'))
ORDER BY mem_name
LIMIT 0 , 30
...
SELECT CONCAT( firstname, ' ', surname ) AS mem_name, f.name AS facility,
CASE
WHEN m.memid =0
...
SELECT mem_name, facility, tot_cost
FROM (

SELECT CONCAT( firstname, ' ', surname ) AS mem_name, f.name AS facility, DATE( b.starttime ) AS DATE,
CASE WHEN b.memid =0
THEN b.slots * f.guestcost
ELSE b.slots * f.membercost
END AS tot_cost
...
WITH y AS (SELECT DISTINCT x.facid,x.name, SUM(x.guest) OVER(PARTITION BY x.facid) AS gsum,
...
SELECT a.firstname||' '||a.surname AS mem_name, b.firstname||' '||b.surname AS rec_name
...
WITH x AS(SELECT facid,  sum(slots) AS tot_slots
FROM Bookings
WHERE memid <> 0
GROUP BY facid
